
namespace Claro.SIACU.Web.WebApplication.Areas.Dashboard.Helpers.Postpaid.LocalTrafficDetail
{
    public class LocalTrafficDetailCallTM
    {
        public LocalTrafficDetailCallTM()
        {
            CALLDURATION = "";
            CALLTIME = "";
            CALLTIMEFIN = "";
            CALLDATEFIN = "";
            CALLNUMBER = "";
            CALLDATE = "";
            CALLDESTINATION = "";
            CALLDURATIONMIN = "";
            CALLTOTAL = "";
            CALLAMBITO = "";
         
        }
        public string CALLDURATION { get; set; }
        public string CALLTIME { get; set; }
        public string CALLTIMEFIN { get; set; }
        public string CALLDATEFIN { get; set; }
        public string CALLNUMBER { get; set; }
        public string CALLDATE { get; set; }
        public string CALLDESTINATION { get; set; }
        public string CALLDURATIONMIN { get; set; }
        public string CALLTOTAL { get; set; }
        public string CALLAMBITO { get; set; }
       
    }
}